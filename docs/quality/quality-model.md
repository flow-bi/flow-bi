...
이 문서는 `flow-bi`에서 Active Plan, Task, Agent 결과와 전체 기능의 품질을 일관되게 판정하기 위한 기준을 정의한다.
...
- Agent가 담당 Task와 수정 가능 경로 안에서만 작업했다.
...
이 절은 작업 유형에 공통으로 적용되는 검증 관점만 정의한다. 기능·도메인별 업무 규칙과 수치 기준은 관련 Product Spec, Design Doc, `SECURITY.md` 및 승인된 ADR을 기준으로 검증하고, Active Plan에서 해당 규칙의 허용·거부·경계 시나리오를 구체화한다.
...
비기능 요구사항의 내용과 목표 수치는 관련 Product Spec이 소유한다. 현재 요구사항의 기준 문서는 다음과 같다.

- 인증과 비밀번호 보호: `docs/product-specs/auth.md`
- 캘린더 응답 시간: `docs/product-specs/calendar.md`
- AI 응답 시간: `docs/product-specs/ai-assistant.md`
- 가용성, PC·Mobile, 확장성: `docs/product-specs/system-quality.md`

비기능 요구사항을 통과로 판정하려면 Active Plan 또는 검증 기록에 최소한 다음 측정 조건을 명시해야 한다.
...
측정 조건이나 요구사항 소유 문서가 확정되지 않은 경우 임의의 환경이나 표본으로 충족을 선언하지 않고 `BLOCKED` 또는 사람 검토 대상으로 기록한다.

## 10. 재시도 정책

- 최초 실행을 포함해 같은 오류당 최대 3회다.
