...
- 회의실 현황: 회의실 목록을 시간대 그리드의 왼쪽 축으로 통합하고, 오른쪽에는 회의실별 예약 현황을 **시간대(9:00~18:00) 가로축**으로 표시한다. 회의실 정보(이름/수용 인원/장비)는 왼쪽 축에만 표시하고, 오른쪽 그리드에는 예약 블록만 표시한다. 예약 블록 표시 정보는 예약 제목, 시간대, 예약 팀이다.
- 검색: 수용인원/날짜/시간대/상태로 검색하며, **필터링이 아니라 우선순위 재정렬** 방식이다 (검색 조건에 안 맞아도 목록에서 사라지지 않고 순서만 밀림).

### 회의실 예약 (FR-020, FR-021)
- 회의실 선택 → 중앙 모달에서 예약 입력: 선택된 회의실, 날짜, 시간대, 참석자(이름 검색 추가), 회의 일정 상세 설명.
- 예약 모달이 열린 상태에서도 대상 회의실을 변경할 수 있다. 회의실 또는 날짜를 바꾸면 기존 `GET /api/v1/rooms/{roomId}/reservations?date=` API를 재사용해 모달 안의 예약 현황을 다시 조회한다.
- 예약 완료 시 `schedules` + `rooms_reservations`가 동시에 생성되어 캘린더에도 표시된다(FR-021, `schedule.md` 참고).
- **진입점은 두 곳**: 회의실 화면의 "예약하기"와 캘린더 "일정 추가" 모달에서 "회의실 일정" 선택 시 모두 동일한 예약 생성 API를 호출한다(`schedule.md` 3번 확정 사항). 백엔드는 진입점을 구분할 필요가 없다.
