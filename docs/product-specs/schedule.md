...
캘린더의 "일정 추가" 모달과 회의실 화면의 "예약하기" 플로우는 **같은 백엔드 로직(회의실 예약 생성)으로 통합**한다.

- 캘린더 "일정 추가" 모달에서 "회의실 일정"을 선택하면, 회의실 선택 UI(회의실 목록 + 시간대별 예약 가능 여부)가 모달 안에서 이어지며, 최종적으로 `meetingroom` 도메인의 `POST /api/v1/rooms/{roomId}/reservations`를 호출한다 (`meeting-room.md` 참고).
- 회의실 화면의 "예약하기"도 동일한 API를 호출한다. 즉 진입점은 두 곳(캘린더, 회의실 화면)이지만 백엔드 엔드포인트와 처리 로직은 하나로 공유된다.
- **도메인 경계 영향 없음**: 이 통합은 백엔드 도메인 의존 방향(`meetingroom → schedule`, ARCHITECTURE.md 3.2)을 바꾸지 않는다. 프론트엔드의 `schedule` feature가 `meetingroom` 도메인의 API를 호출하는 것뿐이며, 백엔드 `schedule` 도메인 코드가 `meetingroom`을 참조하게 되는 것이 아니다.
- 프론트엔드 구현 시, "일정 추가" 모달에서 "기본 일정"과 "회의실 일정" 선택에 따라 다른 API(`POST /api/v1/schedules` vs `POST /api/v1/rooms/{roomId}/reservations`)를 호출하도록 분기한다.
