...
class AuthFeaturePackageStructureTest { // Feature ownership contract.
  ...
  void places_types_in_feature_packages_and_removes_legacy_packages() {
    ...
    assertThat(
        classIsPresent("com.flowbi.domain.auth.login.AuthenticationDependencyUnavailableException"))
        .isTrue();
    ...
    assertThat(classIsPresent("com.flowbi.domain.auth.login.ratelimit.RedisLoginRateLimiter"))
        .isTrue();
    assertThat(
        classIsPresent("com.flowbi.domain.auth.login.ratelimit.LoginRateLimitUnavailableException"))
        .isTrue();
    assertThat(classIsPresent("com.flowbi.domain.auth.password.InitialPasswordChangeController"))
        .isTrue();
    assertThat(classIsPresent("com.flowbi.domain.auth.password.InitialPasswordChangeService"))
        .isTrue();
    ...
    for (String legacyPackage : new String[]{"controller", "service", "dto", "entity", "repository",
        "exception", "ratelimit"}) {
  ...
  void keeps_controller_service_repository_dependency_direction() throws Exception {
    ...
    assertThat(
        hasConstructorDependency("com.flowbi.domain.auth.password.InitialPasswordChangeController",
            "com.flowbi.domain.auth.password.InitialPasswordChangeService"))
        .isTrue();
