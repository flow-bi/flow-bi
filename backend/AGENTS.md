...
- Backend Worker는 기본적으로 `backend/`만 수정한다.
- Frontend, Harness 및 루트 정책은 Active Plan에 명시된 경우에만 변경한다.
...
- 테스트 도구가 미확정인 동안 새 도구를 임의 도입하지 않고 승인된 대체 검증을 기록한다.
...
- Active Plan의 유스케이스와 인수 조건 충족
- TDD 증거와 적용 가능한 전체 테스트 통과
- 인증·인가 및 객체 수준 접근 검증
- 트랜잭션, 상태 전이와 실패 시 데이터 정합성 검증
- 공개 API 또는 DB 변경 시 문서·마이그레이션 동기화
- 로그와 응답에 민감정보가 없음을 확인
- 미실행 검증, 남은 위험과 기술 부채 보고
