...
2.1~2.4절의 관리자 상태·시간 필드와 3절의 역할·권한 영역은 관리자 Product Spec과 인증·권한 Design Doc에 맞춘 **구현 전 Target Contract**다. 현재 PostgreSQL에 적용된 구조로 간주하지 않으며, 실제 테이블 생성·변경은 승인된 Active Plan의 새 Flyway Migration과 PostgreSQL 검증이 완료된 뒤 구현 상태로 전환한다.
...
| `employee_number`   | `VARCHAR(50)`  | UNIQUE, NOT NULL                    | 사번 및 로그인 ID |
...
서버가 직원 등록·관리자 초기화·계정 재활성화용 임시 비밀번호를 생성할 때 `must_change_password`를 `TRUE`로 설정하고, 사용자가 새 비밀번호로 변경을 완료하면 `FALSE`로 변경한다. 평문 임시 비밀번호는 저장하지 않는다.
...
현재 관리자 RBAC는 시스템 전체에 적용되는 고정 역할·권한 카탈로그다. 조직 범위 역할과 런타임 역할·권한 CRUD는 포함하지 않는다. 모든 Surrogate PK는 실제 Migration에서 `BIGINT GENERATED BY DEFAULT AS IDENTITY`를 사용한다.
...
| 컬럼           | 타입     | 제약         | 설명       |
| -------------- | -------- | ------------ | ---------- |
| `user_role_id` | `BIGINT` | PK, IDENTITY | Mapping ID |
| `user_id`      | `BIGINT` | FK, NOT NULL | 사용자 ID  |
| `role_id`      | `BIGINT` | FK, NOT NULL | 역할 ID    |

`UNIQUE(user_id, role_id)`로 같은 전역 역할의 중복 부여를 막는다. 사용자 기준 역할 조회는 이 UNIQUE Index를 사용하고, 역할 보유 사용자 조회를 위해 `role_id` Index를 추가한다. 역할 부여·회수 시각, 행위자와 이전 상태는 DB에 저장하지 않는다.

### 3.5 공통 제약과 기준 데이터

- `roles`, `permissions`, `role_permissions`, `user_roles`의 FK 삭제 정책은 `RESTRICT`다. 사용자·역할·권한 원장을 지우는 대신 필요한 Mapping을 명시적으로 회수한다.
- `roles`와 `permissions`의 `updated_at`은 `DEFAULT CURRENT_TIMESTAMP`만으로 수정 시 자동 갱신되지 않는다. 구현에서는 프로젝트의 `Instant`, `@PreUpdate` 정책으로 변경 시각을 갱신한다.
- Mapping은 상태·시각 갱신 대상이 아니므로 생성·수정·부여 시각 컬럼을 두지 않는다. 역할 부여·회수 전용 감사 테이블과 보안 로그도 만들지 않는다.
- 관리자 Migration은 `ADMIN`, `SYSTEM_ADMIN`, `HR_ADMIN` 역할과 `SYSTEM_MONITOR`, `ACCOUNT_STATUS_READ`, `THEME_MANAGE`, `ROOM_RESOURCE_MANAGE`, `USER_MANAGE`, `TEAM_MANAGE`, `ROLE_MANAGE`, `PRIVILEGED_ACCOUNT_MANAGE` 권한을 고정 기준 데이터로 생성한다.
- `ADMIN`은 8개 권한 전부, `SYSTEM_ADMIN`은 앞의 시스템 운영 4개 권한, `HR_ADMIN`은 `USER_MANAGE`, `TEAM_MANAGE`를 `role_permissions`에 명시적으로 연결한다. 역할 상속이나 `ADMIN` 우회는 없다.
- 최초 `ADMIN` 직원은 배포 환경에서 주입한 사번·이름·이메일·기존 활성 `team_id`·기존 `position_id`·BCrypt 해시로 생성한다. 실제 값은 Git과 문서에 저장하지 않고 누락·중복·잘못된 참조가 있으면 Migration을 실패시킨다. 계정은 `ACTIVE + EMPLOYED`, `must_change_password=TRUE`로 생성한다.
- 관리자 역할은 `ACTIVE + EMPLOYED` 사용자에게만 연결한다. 마지막 `ACTIVE + EMPLOYED + ADMIN` 사용자의 역할 회수·비활성화·퇴직은 애플리케이션 트랜잭션에서 거부한다.
- 역할 또는 주요 권한 변경 시 대상 사용자의 모든 세션을 무효화하고 다음 로그인에서 현재 권한을 다시 로딩한다.
...
- 관리자 Target Contract를 제외한 초기 기준선 영역의 PostgreSQL `DATETIME` 타입 정합성
- `projects_members`, `rooms_reservations`, `schedules_details` 등 명칭과 오탈자
...
- 회의실 중복 예약의 DB 수준 제약
- 일정·프로젝트 등 관리자 Target Contract 밖의 Status 허용값과 상태 전이
- `Field` 예비 컬럼과 회의실 장비 모델
- `capacity`, `level`, `content`, `location` 타입과 길이
- 관리자 Target Contract 밖의 Index, Foreign Key 삭제 정책과 감사 필드 제약
