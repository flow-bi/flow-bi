...
Backend verifier 명령이 실행 중이거나 shell session을 반환한 경우 같은 CLI를 새 shell 명령으로 시작하거나 중복 호출하지 않는다. 기존 실행을 wait/poll해 최종 종료 코드와 출력을 확인한다.
동일 요청은 부모 verifier의 single-flight 실행에 합류하며, 확정 종료 후 실패 원인을 수정했거나 명시적인 재검증이 필요할 때만 다시 실행한다. HTTP 429 같은 실행 중 충돌 응답만으로 검증 실패나 최종 판정을 확정하지 않고, 최종 JSON에는 완료된 최신 실제 결과만 반영한다.
