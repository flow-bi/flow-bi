...
class BackendVerifierTests(unittest.TestCase):
    ...
    def test_formatter_rejects_forbidden_paths_symlinks_and_repository_escape(self) -> None:
        ...
        symlink = self.root / "backend/src/main/java/allowed/Link.java"
        ...
        with BackendVerifier(self.root, runner=runner) as verifier:
            ...
            for paths in (
                ...
                ["backend/src/main/java/allowed/Link.java"],
                ["../outside.java"],
                [],
            ):
                with self.assertRaisesRegex(BackendVerifierClientError, "거부"):
