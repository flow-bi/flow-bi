...
const PLAN_ID = /^[a-zA-Z0-9]+(?:-[a-z0-9]+)*-[0-9]{2}$/;
...
export function validatePrompt(input, projectRoot = PROJECT_ROOT) {
  ...
  if (!PLAN_ID.test(planId)) {
    return block(
      "plan ID는 feature명과 두 자리 번호 형식이어야 합니다 (예: dashboard-01).",
    );
  }
  ...
  if (relativePlanPath.startsWith("..") || isAbsolute(relativePlanPath)) {
    return block(
      "harness plan 경로는 docs/plan/active 밖을 가리킬 수 없습니다.",
    );
  }
